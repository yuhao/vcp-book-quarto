...
A high CG means the output voltage change is very sensitive to small amount of input light change, which is good for improving the signal-to-noise ratio.
In contrast, a small CG means the output voltage change is relatively small given the same amount of light change, and that small voltage change becomes very difficult to detect when there are noises.
...
Such a PD is also called the Pinned Photodiode, or PPD [@fossum2014review].], another capacitive area, and is sometimes called the **measurement node**, the *sense node*, or the *conversion node*, because that is where the charges are actually being measured.
...
A common strategy toward HDR is called **exposure bracketing**, which can be implemented in two ways, both involving taking multiple shots of the scene and then fuse them.
...
The sensitivity is usually controlled by pixel size: a large pixel can collect more charges per unit light illuminance than a small pixel simply because of the large surface area.
So the larger group of pixels saturate faster.
The two groups of pixels are distributed over the sensor plane, so they each performs a uniform sampling of the scene.

Another approach is the time-to-saturation (TTS) technology [@stoppa2002novel], which uses a counter to measure the time it takes for each pixel to saturate and use that time to extrapolate the luminance information given the actual exposure time:
