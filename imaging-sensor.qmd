...
#### Pixel Well and Dynamic Range {#sec-chpt-imaging-sensor-pixel-charge-dr}
...
The **full-well capacity** (FWC) is the max amount of electrons that can be held by a pixel.
...
#### High Dynamic Range Imaging {#sec-chpt-imaging-sensor-pixel-charge-hdr}
...
Computationally, the common strategy is called **exposure bracketing**, which can be implemented in two ways, both involving taking multiple shots of the scene and then fuse them.

* Each shot can have the same, short exposure time so no pixel is over-exposed. We then average multiple shots; averaging is a form denoising (@sec-chpt-imaging-noise-??).  This is the approach that Google's HDR+ system takes [@hasinoff2016burst].
* Or each shot can have a different exposure time.  Long-exposure shots are used to capture details in low-luminance regions, and short-exposure shots capture details in high-luminance regions.

Either way, the issue with exposure bracketing is the longer capturing time, and the resulting image is more susceptible toward motion blur.
We ideally would like "single-shot" HDR.
There are multiple approaches, and they usually require co-designing the image sensor/pixel design with the post-processing algorithms.

One strategy is to use the so-called "split-pixel" technology, an emerging technology that has not quite made its way into products but nevertheless interesting.
The idea is to use two or multiple groups of pixels, each with a different sensitivity to light.
This sensitivity can be controlled by changing the FWC of the pixels: a large FWC is less likely to saturate during the same exposure time that does a smaller pixel.
The two groups of pixels can then be distributed over the sensor plane, so they each performs a uniform sampling of the scene.
...
where $\mathcal{Q}_{sig}$ is the charge in the signal stored in the capacitor, $C_{FD}$ is the capacitance, and $g$ is the voltage gain of whatever device is used to read out the voltage (usually a source follower; see later).
...
Such a PD is also called the Pinned Photodiode, or PPD [@fossum2014review].], another capacitive area, and is sometimes called the *measurement node* or the *sense node*, because that is where the charges are actually being measured.
...
![(a): circuit diagram of a typical 4T pixel design; adapted from @ma2024efficient[Fig. 2.5(a)]. (b): timing diagram of operating a 4T pixel.](figs/4taps){#fig-4taps width="100%"}
...
A precursor to APS was the \textbf{Passive Pixel Sensor} (PPS), first suggested in @weckler1967operation and @dyck1968integrated.
...
![(a): analog CDS and programmable amplifier; from @ma2024efficient[Fig. 2.5(b)]. (b): a single-slope ADC typically used in image sensors; adapted from @ma2024efficient[Fig. 2.5(c)].](figs/readout){#fig-readout width="100%"}
...
See @fossum1993active, @fossum1997cmos, and @el2005cmos for the historical background and comparisons.
...
We will discuss the calibration and the post-processing pipeline in greater details in @sec-???.
...
We will have more to say about the demosaicing process when we get to @sec-chpt-imaging-sensor-isp, but for now, let's just observe that demosaicing is nothing more than a signal sampling and reconstruction problem.
