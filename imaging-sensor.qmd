...
* In the first HCG read-out, we do not use $FD_2$.
This reading has a high HCG and high SNR, which is especially important for dark parts of the scene.
However, in the bright areas, $FD_1$ saturates, and this reading is useless for highlights.
Importantly, however, the overflowing charges are not discarded; they still stay in the PD.
* Then in the subsequent LCG read-out, the extra $FD_2$ is switched in.
Now all the charges, including the left-over charges in the PD and the charges in $FD_1$, are then re-distributed to $FD_1$ *and* $FD_2$, which collectively will not saturate, so highlights are captured at the cost of low SNR.

<!-- Fig. 7-8 in @iida20180 are great examples. -->
...
![Illustration of how split pixel (with a group of large pixels, LP, and a group of small pixels, SP) and dual conversion gain techniques extend the dynamic range. Redrawn from @xu2022analysis.](figs/sp_dcg_dr){#fig-sp_dcg_dr width="60%"}

One strategy is to use the so-called **split-pixel** technology, an emerging technology that sensor companies are exploring.
The idea is to use two or multiple groups of pixels, each with a different "sensitivity" to light [@iida20180; @solhusvik20191280; @willassen20151280; @xu2022analysis].
The sensitivity is usually controlled by pixel size (and microlens size): a larger pixel can collect more charges per unit incident luminance than a smaller pixel simply because of the large photon collection area.
Therefore, given the same scene luminance, larger pixels saturate faster.
The two groups of pixels are interleaved on the sensor plane, so they each perform a uniform sampling of the scene (preceded by a spatial integration over the pixel area of course).

The way that split pixels extend dynamic range is illustrated in @fig-sp_dcg_dr.
Large pixels saturate at a low luminance level $L_1$, so only pixels that image low-luminance regions in the scene do not saturate; as a result, large pixels provide a good sampling of the low-luminance information (or rather, low-luminance information is adequately sampled by large pixels).
In contrast, small pixels saturate at a high luminance level $L_2$, so they provide a good sampling for high-luminance information in the scene.

If we increase the FWC of the small pixels, they take even longer/higher luminance to saturate.
The way we increase the FWC is by adding a lateral overflow integration capacitor (**LOFIC**), which holds the overflow charges from the photodiode of a small pixel during exposure [@sugawa2005100; @akahane2006sensitivity; @takayanagi2019120; @ikeno20224].
In almost all cases, the FD itself participates in collecting the overflow charges, too.
In this way, the FWC of the small pixels is effectively the capacity of the photodiode itself plus that of the LOFIC and the FD.
