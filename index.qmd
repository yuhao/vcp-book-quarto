...
Visual computing is a wonderfully broad field, touching everything from the biology of human vision to the engineering of sensors, optics, displays, and rendering.
...
What they do is to *offer perspectives*, and that always reveals the idiosyncrasies of the author.

The perspective I most want to convey is this: every conclusion is drawn based on set of assumptions.
By taking a first-principles approach, I aim to make those assumptions explicit, so it is always clear when each conclusion applies and when it does not.
...
In imaging and display, we review basic principles while being intentionally light on implementation details --- these evolve quickly and are often proprietary.

In rendering, we focus on the classical computational models of light-matter interactions while completely ignoring the implementation issues such as the modern graphics pipelines (e.g., Vulkan) and GPU hardware.
We will not cover the emerging (neural) rendering paradigm in detail, although we will explore its connections to the classical theories.

# Other Books

This is necessarily an incomplete list, because I can mention only books that I find myself reviewing from time to time.

For human vision, the monumental book by @wandell1995foundations is a must read (a second version is in the works, I was told).
...
None of the three books above has complicated equations --- they focus on building intuitions and basic principles.
...
@sharma2003digital and @nakamura2006image are two slightly dated but excellent references; the latter is heavier on the hardware implementations.