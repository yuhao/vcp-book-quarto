...
## A Little Bit of Radiometry and Photometry {#sec-chpt-mat-basics-radiometry}
...
Irradiance is a more primitive measure than power, because we can derive the power of a surface by integrating the irradiance over the surface area:
...
Irradiance has a unit of $W/m^2$, and *spectral* irradiance has a unit of $W/(m^2 \cdot nm)$.
...
Radiance has a unit of $W/(m^2\cdot sr)$, and *spectral* radiance has a unit of $W/(m^2\cdot sr \cdot nm)$.
...
Looking at the effective area in @fig-radiance, if the irradiance at the infinitesimal area $p$ is $\d E(p)$, the irradiance at the (infinitesimal) effective area, which is projected from $\d A$ along $\omega$, is $\frac{\d E(p)}{\cos\theta}$.
Combining this with @eq-radiance_3, we can interpret the radiance $L(p, \omega)$ differently: it is the ratio between $\d E_\bot(p)$, the infinitesimal irradiance defined at the surface perpendicular to the light direction, and the infinitesimal solid angle.
This is shown in @eq-radiance_4.
