...
![Contrast sensitivity function (CSF) under an ON-center midget RGC ^[This is actually a parvocellular LGN neuron, which is directly projected from the midget RGC and shares the same receptive field with that of the midget RGC.]; the filled circle is the represents the sensitivity of a uniform signal (i.e., 0 Hz). CSF is bandpass. It is worth emphasizing that the spatial frequency on the $x$-axis is expressed in cycles per degree (CPD), which describes the signal frequency on the retina, which is what matters for vision. Depending on the viewing distance, the same world-space signal can result in different retinal signals. Adapted from @derrington1984spatial[Fig. 3C].](figs/csf_new){#fig-csf width="80%"}
