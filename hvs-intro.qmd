...
With a spatial frequency of about 5 CPD, the positive amplitude coincides with the ON-center of the cell and the negative amplitude coincides with the OFF-surround of the cell.
...
One thing subtle to note is that the Michelson contrast of a signal, as defined in @eq-michelson_contrast, is bounded between 0 and 1, so long as the signal is positive everywhere (which is of course the case in real-world visual signals).
...
@fig-csf quantifies the spatial frequency of a signal using **Cycle Per Degree** (CPD), which is the number of cycles/periods in a degree.
...
$S_1$ and $S_2$ are two different real-world objects, but they produce identical retinal signals (assuming the eye optics is approximated as a pinhole system) and, of course, have the same CPD.
In contrast, $S_3$ is a signal produced by moving $S_2$ closer to the eye; even though they have the same ordinary spatial frequency, but clearly the former produces a retinal signal that has a higher CPD than that of the latter.
Since the retinal signal is what matters for vision, using CPD as a spatial frequency measure is clearly more relevant.
